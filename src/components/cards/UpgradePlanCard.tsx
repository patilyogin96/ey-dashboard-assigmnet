...
const UpgradePlanCard = ({ plan }: UpgradePlanCardProps) => {
  return (
    <Card className="w-full shadow-sm border-gray-100 max-h-128 flex flex-col py-5">
      ...
      <CardContent className="space-y-6 px-5 overflow-y-auto">
          ...
          {plan.paymentMethods.map((method) => (
            <div key={method.id} className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-3 flex-1">
                <div className={`w-12 h-8 ${method.cardColor} rounded-md flex items-center justify-center text-white shadow-sm`}>
        ...
        <Button className="w-full bg-purple-600 hover:bg-purple-700 text-white h-14 text-[15px] font-semibold tracking-wide rounded-xl">
          CONTECT NOW
        </Button>
      </CardContent>
    </Card>
